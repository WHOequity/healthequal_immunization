...
## Limit data to specific indicator/dimensions and completeness
data_imm <- df_imm %>% 
  ...
  # Latest situation (2013-2022)
  group_by(setting, indicator_abbr) %>%
  mutate(max_date=max(date)) %>%
  ungroup() %>%
  mutate(recent = date==max_date, 
         latest = ifelse(recent==TRUE & date>=2013, 1, 0)) %>%
  filter(latest==1) %>% 
  select(-max_date)
...
summary_ordered <- cbind(setting=rep(measures_ordered$setting,1),
                ...
                rbind(measures_ordered$d, measures_ordered$r,
                      measures_ordered$aci, measures_ordered$rci,
                      measures_ordered$sii, measures_ordered$rii,
                      measures_ordered$par, measures_ordered$paf)) %>%
  arrange(setting, date, indicator, measure)
...
summary_nonordered <- cbind(setting=rep(measures_nonordered$setting,1),
                       ...
                       bind_rows(measures_nonordered$d, measures_nonordered$r,
                                 measures_nonordered$bgv, measures_nonordered$bgsd,
                                 measures_nonordered$cov, measures_nonordered$mdmu,
                                 measures_nonordered$mdmw, measures_nonordered$idisu,
                                 measures_nonordered$idisw, measures_nonordered$ti,
                                 measures_nonordered$par, measures_nonordered$paf)) %>%
  arrange(setting, date, indicator, measure)
