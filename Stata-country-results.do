...
//	Prepare immunization dataset from the Health Inequality Data Repository (HIDR)
	...
	*Latest situation (2013-2022)
	...
	drop max
	gen latest=0
	replace latest=1 if recent==1 & date>=2013
	keep if latest==1
	save "immunization-data", replace
...
//	Prepare Global Data Lab dataset from the Health Inequality Data Repository
	...
	*Order subnational regions
	...
	save "subnational-hdi-data", replace
	
//	Append immunization and HDI datasets

	use "immunization-data", clear
	append using "subnational-hdi-data"
	save "data", replace
...
//	Calculate summary measures of health inequality 
	...
	*Binary dimension (place of residence)
	...
	foreach m of local measure {
		preserve
			...
			keep setting date source indicator_abbr indicator_name dimension setting_average iso3 whoreg6 wbincome count `m'_*
			duplicates drop
	...
	*Nonordered dimension (subnational region)
	use "data", clear
